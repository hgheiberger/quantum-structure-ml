...
run_name = (time.strftime("%y_%m_%d-%H_%M", time.localtime()))

#Used for debugging
identification_tag = f"This one Trial 1 {run_name}"
...
def train(model, optimizer, dataloader, dataloader_valid, max_iter=50., device="cpu"):
    model.to(device)
...
for results in train(model, opt, dataloader, dataloader_valid, device=device, max_iter=50):
...
for i, index in enumerate(index_tr):
    ...
    with open('training_results.txt', 'a') as f:
                f.write(f"{id_list[index]} {compound_list[index]} Prediction: {output} Actual: {d.y.item()} \n")
...
with open('statistics.txt', 'a') as f:
    ...
    f.write(f"Identification tag: {identification_tag}i\n")
    f.write('Average precision-recall score: {0:0.2f} \n'.format(
      average_precision))
    f.write('F1 Score: {0:0.2f} \n'.format(
      f1_score))      
